...
import { contentIntelligenceBriefUpdateSchema } from "@/lib/validation/content-intelligence";
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  const body = parseBody(contentIntelligenceBriefUpdateSchema, await jsonBody(request));

  return withContentEdit(request, organisationId, async ({ requestId, tenant }) => {
    const session = await contentIntelligenceGenerationService.updateBrief(
      brandId,
      organisationId,
      contentId,
      body,
