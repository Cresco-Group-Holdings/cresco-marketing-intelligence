...
import { visualExportCreateSchema } from "@/lib/validation/visual-studio";
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  const body = parseBody(visualExportCreateSchema, await jsonBody(request));
  return withVisualStudioWrite(request, organisationId, async ({ requestId, tenant }) =>
    apiSuccess(
      await visualStudioService.exportProject(
        brandId,
        organisationId,
        visualProjectId,
        body,
