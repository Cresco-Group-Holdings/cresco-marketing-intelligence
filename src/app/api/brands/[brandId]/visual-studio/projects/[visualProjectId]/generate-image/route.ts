...
import { visualAiImageSchema } from "@/lib/validation/visual-studio";
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  const body = parseBody(visualAiImageSchema, await jsonBody(request));
  return withVisualStudioWrite(request, organisationId, async ({ requestId, tenant }) =>
    apiSuccess(
      {
        generation: await visualStudioService.generateImage(
          brandId,
          organisationId,
          visualProjectId,
          body,
