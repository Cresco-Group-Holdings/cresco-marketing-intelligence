...
export type TaskDisplayInput = Pick<TaskSnapshot, "status" | "dueDate" | "deferredUntil">;
...
export function validateTaskTransition(task: TaskSnapshot, toStatus: CrmTaskStatus): TransitionResult {
  if (task.status === toStatus) return { valid: true, errors: [], nextStatus: toStatus };
  ...
  return { valid: true, errors: [], nextStatus: toStatus };
}

export function isTaskOverdue(task: TaskDisplayInput, now = new Date()): boolean {
...
export function resolveDisplayStatus(task: TaskDisplayInput, now = new Date()): CrmTaskStatus {
