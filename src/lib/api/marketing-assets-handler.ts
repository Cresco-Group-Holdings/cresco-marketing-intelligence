...
export function withMarketingAssetsRead(
  request: NextRequest,
  ...
  return withApiHandler(request, handler, {
    organisationId,
    permission: PERMISSIONS["marketingAssets.read"],
  });
}

export function withMarketingAssetsWrite(
  request: NextRequest,
  organisationId: string,
  handler: Parameters<typeof withApiHandler>[1],
) {
  return withApiHandler(request, handler, {
    organisationId,
    permission: PERMISSIONS["marketingAssets.update"],
  });
}
