...
export function withProviderConnectionsRead(
  request: NextRequest,
  ...
  return withApiHandler(request, handler, {
    organisationId,
    permission: PERMISSIONS["providerConnections.read"],
  });
}

export function withProviderConnectionsWrite(
  request: NextRequest,
  organisationId: string,
  handler: Parameters<typeof withApiHandler>[1],
) {
  return withApiHandler(request, handler, {
    organisationId,
    permission: PERMISSIONS["providerConnections.create"],
  });
}

export function withProviderConnectionsAuthorize(
  request: NextRequest,
  organisationId: string,
  handler: Parameters<typeof withApiHandler>[1],
) {
  return withApiHandler(request, handler, {
    organisationId,
    permission: PERMISSIONS["providerConnections.authorize"],
  });
}
