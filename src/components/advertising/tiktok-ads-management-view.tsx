...
import { TIKTOK_ADS_CAPABILITIES } from "@/lib/advertising-providers/capability-gates";

export type TikTokAdsManagementViewMode = "overview" | "drafts" | "launches";

function TikTokNav({ active }: { active: TikTokAdsManagementViewMode }) {
  const tabs = [
    { mode: "overview" as const, label: "Overview", href: "/advertising/tiktok" },
    { mode: "drafts" as const, label: "Drafts", href: "/advertising/tiktok/drafts" },
    { mode: "launches" as const, label: "Launches", href: "/advertising/tiktok/launches" },
  ];
  return (
    <AdvertisingSectionNav
      ariaLabel="TikTok Ads navigation"
      tabs={tabs.map((tab) => ({
        label: tab.label,
        href: tab.href,
...
export function TikTokAdsManagementView({ mode }: { mode: TikTokAdsManagementViewMode }) {
  const { preference } = useWorkspace();
  const brandId = preference.currentBrandId;
  const organisationId = preference.currentOrganisationId;
  const base = brandId ? `/api/brands/${brandId}/advertising/tiktok` : null;
  ...
  const assignAccount = async () => {
    if (!base || !organisationId || !advertiserId) return;
    setLoading(true);
    setMessage(null);
    try {
      await apiFetch(`${base}/accounts?organisationId=${organisationId}`, {
        method: "POST",
        body: JSON.stringify({ action: "assign", advertiserId, pixelId: pixelId || undefined }),
      });
      setMessage("TikTok advertiser assigned.");
      await loadStatus();
    } catch (e) {
  ...
  const disabledCaps = TIKTOK_ADS_CAPABILITIES.filter((c) => !c.available);

  return (
    <div>
      <PageHeader title="TikTok Ads" description="Controlled TikTok campaign management with capability gates and exact-plan approval." />
      <TikTokNav active={mode} />
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-4 space-y-2">
                <Input label="Advertiser ID" placeholder="Advertiser ID" value={advertiserId} onChange={(e) => setAdvertiserId(e.target.value)} />
                <Input label="Pixel ID (optional)" placeholder="Pixel ID (optional)" value={pixelId} onChange={(e) => setPixelId(e.target.value)} />
