...
export function UserMenu({ email, displayName }: UserMenuProps) {
  ...
  return (
    <div className="relative group">
      <button
        type="button"
        className="inline-flex items-center gap-2 rounded-lg border border-border bg-surface-elevated px-3 py-2 text-sm font-medium text-foreground-muted hover:bg-surface-hover"
        aria-haspopup="menu"
      >
      ...
      <div className="invisible absolute right-0 z-40 mt-2 w-56 rounded-lg border border-border bg-surface-elevated p-2 opacity-0 shadow-lg transition group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">
        ...
        <div className="py-1">
          <Link
            ...
            className="flex items-center gap-2 rounded-md px-3 py-2 text-sm text-foreground-muted hover:bg-surface-hover hover:text-foreground"
          >
